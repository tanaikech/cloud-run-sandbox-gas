...
- Medium: [https://medium.com/google-cloud/taking-advantage-of-cloud-run-sandboxes-with-google-apps-script-for-google-workspace-560348c06f40](https://medium.com/google-cloud/taking-advantage-of-cloud-run-sandboxes-with-google-apps-script-for-google-workspace-560348c06f40)
- DEV.to: [https://dev.to/gde/taking-advantage-of-cloud-run-sandboxes-with-google-apps-script-for-google-workspace-5fc5](https://dev.to/gde/taking-advantage-of-cloud-run-sandboxes-with-google-apps-script-for-google-workspace-5fc5)
- LinkedIn: [https://www.linkedin.com/posts/tanaike_taking-advantage-of-cloud-run-sandboxes-with-activity-7503645737461858304-isoz?utm_source=share&utm_medium=member_desktop&rcm=ACoAAGPNwa0BAjsU3wvkp7Mhq6PmtDubnJQpAw8](https://www.linkedin.com/posts/tanaike_taking-advantage-of-cloud-run-sandboxes-with-activity-7503645737461858304-isoz?utm_source=share&utm_medium=member_desktop&rcm=ACoAAGPNwa0BAjsU3wvkp7Mhq6PmtDubnJQpAw8)
...
However, many everyday automation tasks across Google Workspace—such as mathematical recalculations, string parsing, regular expression evaluation, and isolated shell commands—require **deterministic, sub-second execution without the conversational latency (seconds to tens of seconds) or token quota consumption (e.g. 200k TPM limits) of LLM prompts**. Encountering Romin Irani's hands-on guide sparked an immediate insight: _What if we connect this lightweight gVisor micro-sandbox directly to Google Apps Script?_
...
- [Taking Advantage of Cloud Run Sandboxes with Google Apps Script for Google Workspace](#taking-advantage-of-cloud-run-sandboxes-with-google-apps-script-for-google-workspace)
    - [Published articles](#published-articles)
    - [💡 Project Inspiration \& Context](#-project-inspiration--context)
  ...
  - [3. Cost Model \& Safety Guardrails (Zero-Idle \& Always Free)](#3-cost-model--safety-guardrails-zero-idle--always-free)
    - [3.1. Service Free Tier Allocations \& Zero-Idle Cost Architecture](#31-service-free-tier-allocations--zero-idle-cost-architecture)
    - [3.2. 4 Built-In Safety Guardrails](#32-4-built-in-safety-guardrails)
  - [4. Prerequisites \& gcloud CLI Setup](#4-prerequisites--gcloud-cli-setup)
    - [4.1. Prepare Google Cloud Project](#41-prepare-google-cloud-project)
    - [4.2. Install Google Cloud SDK (`gcloud` CLI)](#42-install-google-cloud-sdk-gcloud-cli)
      - [A. Linux (Ubuntu / Debian)](#a-linux-ubuntu--debian)
      - [B. macOS (Homebrew)](#b-macos-homebrew)
      - [C. Windows](#c-windows)
      - [D. Google Cloud Shell (No Installation Required — Recommended)](#d-google-cloud-shell-no-installation-required--recommended)
    - [4.3. Initial Authentication](#43-initial-authentication)
  - [5. Step 1: One-Click Cloud Run Deployment](#5-step-1-one-click-cloud-run-deployment)
    - [5.1. Specifying the Project ID](#51-specifying-the-project-id)
      - [Method A: Prefix Command with PROJECT\_ID (★ Recommended \& Safest)](#method-a-prefix-command-with-project_id--recommended--safest)
      - [Method B: Set Default gcloud Project](#method-b-set-default-gcloud-project)
      - [Method C: Interactive Terminal Prompt](#method-c-interactive-terminal-prompt)
    - [5.2. Automated Actions Performed by `deploy.sh`](#52-automated-actions-performed-by-deploysh)
      - [(Optional) Quick Terminal Smoke Test](#optional-quick-terminal-smoke-test)
  ...
  - [7. Step 3: Test Execution \& Verification Telemetry](#7-step-3-test-execution--verification-telemetry)
    - [① Health Check (`checkHealth`)](#-health-check-checkhealth)
    - [② Execute All Test Vectors (`runAllTests`)](#-execute-all-test-vectors-runalltests)
  - [8. Step 4: Complete Resource Cleanup (100% Zero Residuals)](#8-step-4-complete-resource-cleanup-100-zero-residuals)
    ...
    - [② Cloud Resource Complete Purge](#-cloud-resource-complete-purge)
      - [What `cleanup.sh` Purges:](#what-cleanupsh-purges)
  ...
  - [11. Strategic Comparison: Cloud Run Sandboxes vs. Gemini Managed Agents](#11-strategic-comparison-cloud-run-sandboxes-vs-gemini-managed-agents)
    - [Architectural Trade-offs (Pros \& Cons)](#architectural-trade-offs-pros--cons)
    - [Practical Scenarios: When to Use Which Architecture](#practical-scenarios-when-to-use-which-architecture)
...
_Figure 1: Architectural workflow linking Google Apps Script, Cloud Run FastAPI Runner, and gVisor Micro-Sandbox._
...
_Figure 2: Google Cloud Always Free tier allocations and zero-idle cost architecture for Cloud Run Sandboxes._
...
_Figure 3: Latency breakdown between cold-start container provisioning (TC-01: 3,609 ms) and warm executions (TC-02 to TC-08: 322–539 ms)._
...
_Figure 4: Comprehensive 8-axis test suite matrix evaluating deterministic execution and security isolation._
...
2. **Dispatch to Sandbox**: Apps Script reads the table range, formats the data as JSON, and dispatches it with a Python script utilizing `pandas`, `matplotlib`, and `seaborn` to `POST /run`.
   - _Note_: This visualization script can be pre-authored or **dynamically synthesized on demand by the Gemini API** from natural language instructions.
...
_Figure 5: Google Sheets generated with multivariate student data and an embedded correlation heatmap rendered by Cloud Run Sandboxes (Matplotlib & Seaborn)._
...
_Figure 6: Strategic workload comparison: Cloud Run Sandboxes vs. Gemini Managed Agents._

![Compute Specifications & Session Lifecycle](images/comparison_specs.png)
_Figure 7: Detailed compute specifications, hardware profiles, and session lifecycle comparison._
...
- **Inspiration & Prior Articles**:
  - Romin Irani: [_Safely Running Untrusted Code: A Hands-On Guide to Google Cloud Run Sandboxes_](https://medium.com/google-cloud/safely-running-untrusted-code-a-hands-on-guide-to-google-cloud-run-sandboxes-8bbc95d391c7) (Medium, 2024)
  - Kanshi Tanaike: [_A Fake Sandbox for Google Apps Script: A Feasibility Study on Securely Executing Code Generated by LLMs_](https://medium.com/google-cloud/a-fake-sandbox-for-google-apps-script-a-feasibility-study-on-securely-executing-code-generated-by-cc985ce5dae3) (Medium, 2024)
  - Kanshi Tanaike: [_Exploring Sandboxing for AI-Generated Google Apps Script_](https://medium.com/google-cloud/exploring-sandboxing-for-ai-generated-google-apps-script-0652a68eb4b7) (Medium, 2024)
  - Kanshi Tanaike: [_Taking Advantage of Gemini Managed Agents with Google Apps Script_](https://medium.com/google-cloud/taking-advantage-of-gemini-managed-agents-with-google-apps-script-19215ab8c61f) (Medium, 2024)
  - Kanshi Tanaike: [_ggsrun - CLI Tool for Google Apps Script Execution_](https://github.com/tanaikech/ggsrun)
